...
app.post('/exclude', (req, res) => {
  try {
    ...
    if (fs.existsSync(exclusionFile)) {
      ...
      content = 'Consultant,Project\n';
    }
    
    const newExclusion = `${consultant},${project}\n`;
    if (!content.includes(newExclusion)) {
...
function loadFilterFiles(files, textInputs = {}) {
  ...
  try {
    ...
    if (engineersList.length > 0) {
      filters.engineers = [...new Set(engineersList)]; // Remove duplicates
    }
    
    // Exclusions filter - combine file and text input
    ...
    // Solution Architects filter - combine file and text input
    let saList = [];
    if (files.saFile) {
      const saContent = fs.readFileSync(files.saFile[0].path, 'utf8');
      saList = saContent.split('\n').map(line => line.trim()).filter(line => line);
    }
    ...
    if (saList.length > 0) {
      filters.solutionArchitects = [...new Set(saList)]; // Remove duplicates
    }
  } catch (error) {
